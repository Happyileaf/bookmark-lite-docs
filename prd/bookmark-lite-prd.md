...
| 字段 | 必填 | 说明 |
|---|---|---|
| 标签名称 | 是 | 标签展示名称，同一数据范围内不可重复（用户级范围按当前用户隔离，应用级范围为全应用） |
| 标签颜色 | 否 | 用于区分标签的视觉标识 |
| 标签描述 | 否 | 标签用途说明 |
| 书签数量 | 是 | 当前标签关联的书签数量 |
| 创建时间 | 是 | 标签创建时间 |
...
产品需要同时满足两类核心场景：访客可直接浏览和使用应用级书签，已登录用户可拥有并管理自己的个人书签。为此，系统定义应用级数据和用户级数据两种数据范围。划分数据范围的目的仅是区分数据来源与管理权限，不改变书签与标签的功能模型。
...
- 普通用户或超级管理员将应用级书签保存到个人书签库时，系统必须创建一份新的用户级个人书签副本，不建立引用关系。
- 保存时默认复制公开书签的标题、URL 和描述；收藏状态、置顶状态、访问时间等个人化状态按用户级默认值初始化。
- 保存成功后，该个人书签副本归当前用户所有，仅当前用户可查看和管理。
- 保存后的个人书签副本与原应用级书签互不联动：后续任一方编辑、隐藏、删除都不影响另一方。
- 保存公开书签时，支持选择个人标签；应用级标签不会自动成为用户级个人标签。
- 如果用户选择沿用公开书签的标签名称，系统应在当前用户个人标签中查找同名标签；存在则复用，不存在则创建同名个人标签后再关联。
- 如果当前用户个人书签库中已存在相同 URL，系统不创建副本，并提示该 URL 已存在且可前往编辑已有书签。

#### 4.4.6 设置字段分类与默认值回退规则

- 应用级设置与用户级个人设置是两份完全独立的数据，分别属于应用级数据域和用户级数据域，禁止互相读取或互相作为兜底值。
- 设置字段分为两类：系统设置字段（主题设置、回收站设置、审计设置）与非系统字段（除系统设置字段外的业务字段，如账户邮箱展示、密码修改相关字段）。
- 系统必须维护一份系统默认设置，用于系统设置字段的统一兜底默认值。
- 非系统字段按各自业务逻辑初始化和更新，例如邮箱与密码相关信息读取用户账户数据。
- 系统设置字段在应用设置和用户设置中初始化为“空值（未设置）”，仅在用户主动设置后写入对应数据域。
- 读取系统设置字段时，若当前数据域字段值为空，则回退读取系统默认设置值；若字段值非空，则使用当前数据域值。
- 读取过程中禁止访问另一数据域的设置值。
...
- 角色采用 RBAC，两类角色为普通用户和超级管理员；超级管理员在普通用户能力基础上额外拥有应用级书签和应用级标签的管理权限。
- 书签和标签通过数据范围区分应用级数据与用户级个人数据，两类数据字段结构和关联规则保持一致。
- 书签与标签通过书签标签关联对象建立多对多关系，允许书签无标签，也允许标签下关联多个书签。
- 应用级书签保存到个人库时，会创建新的用户级个人书签副本，副本与原公开书签之间不建立持续引用关系。
...
|---|---|---|---|
| 直接访问站点域名 | 应用书签页 | 浏览、搜索、筛选、打开、复制公开书签 | 点击登录/注册后进入账户流程 |
| 外部链接进入公开书签页 | 应用书签页（可带查询参数） | 浏览公开内容 | 若访问用户级或管理入口，跳转登录页 |
| 未登录访问受限 URL（用户页/管理页） | 登录页 | 登录或注册 | 登录成功后进入用户书签页 |

访客典型流转：

`访客进入 -> 应用书签页 -> 浏览/检索公开书签 -> 登录或注册 -> 进入用户书签页`
...
| 从公开书签执行“保存到个人库” | 留在应用书签页并提示结果 | 选择标签并保存；若 URL 已存在则提示并引导编辑已有书签 | 可跳转用户书签页查看结果 |
...
| 从 Header 进入“应用管理” | 应用书签管理页 | 维护应用级书签与标签 | 返回公开书签展示页验证效果 |
| 从公开书签页进入应用管理 | 应用书签管理页 | 新增/编辑/删除公开数据 | 回到公开书签页验证展示状态 |

超级管理员典型流转：

`登录 -> 用户书签页 -> 应用书签管理页 -> 应用书签页(验证展示) -> 返回管理页继续维护`

#### 5.1.5 展示场景与管理场景双向流转

| 当前场景 | 触发入口 | 目标场景 | 流转规则 |
|---|---|---|---|
| 应用书签页（展示） | Header 管理入口（管理员可见） | 应用书签管理页（管理） | 仅超级管理员可达 |
| 用户书签页（展示） | Header/按钮“进入管理” | 用户书签管理页（管理） | 保留当前标签与搜索上下文 |
| 用户书签管理页（管理） | Header/按钮“返回书签” | 用户书签页（展示） | 恢复最近一次展示条件 |
| 应用书签管理页（管理） | Header/按钮“查看公开页” | 应用书签页（展示） | 用于即时验证发布结果 |
...
展示场景用于浏览和访问书签，包括应用书签页和用户书签页。该场景中，下左区域展示聚合视图和标签列表，下右区域展示当前选择范围内的书签。
...
- 下左区域在聚合视图下方展示标签列表，标签来源于当前数据范围：应用书签页展示公开标签，用户书签页展示个人标签。
...
| 应用数据展示 | 应用书签页 | 访客、普通用户、超级管理员 | 面向所有访客开放的应用级书签展示页，无需登录即可访问 |
| 应用数据管理 | 应用书签管理页 | 超级管理员 | 超级管理员管理应用级书签与公开标签数据的页面 |
| 应用数据管理 | 应用标签管理页 | 超级管理员 | 超级管理员管理应用级标签信息（创建、编辑、删除） |
| 应用数据管理 | 应用导入导出页 | 超级管理员 | 超级管理员处理应用级书签数据导入、导出 |
...
- 新建书签时，URL 和标题为必填项。
- URL 必须符合有效链接格式。
- 同一数据范围内，书签 URL 必须唯一；重复 URL 不允许创建副本（用户级范围按当前用户隔离，应用级范围为全应用）。
- 用户可以为书签选择一个或多个标签。
- 用户可以在新建书签时同步创建新标签。
...
- 修改 URL 时需要重新校验链接格式。
- 修改后的 URL 若与同一数据范围内已有书签重复，则不允许保存（用户级范围按当前用户隔离，应用级范围为全应用）。
- 修改标签时支持新增关联和移除关联。
...
- 标签名称必填。
- 标签名称在同一数据范围内不可重复（用户级范围按当前用户隔离，应用级范围为全应用）。
- 标签名称需要限制最大长度。
- 支持设置标签颜色。
- 创建成功后，标签出现在侧边栏标签列表中。
...
- 导入重复 URL 时直接跳过：不允许覆盖或创建副本，重复项按失败记录并继续导入其他记录。
...
- 应用设置用于配置应用级数据域内的设置项，仅超级管理员可修改。
- 用户设置用于配置当前登录用户数据域内的设置项，仅当前用户本人可修改。
- 应用设置与用户设置是两份独立数据，禁止互相读取。
- 主题设置、回收站设置、审计设置属于系统设置字段。
- 系统设置字段在各数据域初始化为空值，用户主动设置后才写入对应数据域。
- 读取系统设置字段时，如当前数据域字段值为空，则使用系统默认设置值；禁止从另一数据域继承。

### 6.9.5 系统默认设置

#### 功能说明

系统维护一份全局系统默认设置，用于系统设置字段在任一数据域为空值时的统一兜底。

#### 需求规则

- 系统默认设置独立于应用设置和用户设置存储，不属于应用设置数据域或任一用户设置数据域。
- 系统默认设置仅用于读取时兜底，不自动回写到应用设置或用户设置。
- 系统默认设置字段作为系统内部数据，只支持通过数据库操作修改，不提供前端页面或普通业务接口修改能力。
- 系统设置字段读取顺序为：当前数据域字段值 -> 系统默认设置值。
- 系统默认设置字段至少包含主题设置、回收站设置、审计设置。
- 系统默认设置值如下：

| 系统设置字段 | 系统默认值 | 说明 |
|---|---|---|
| 界面主题 | 跟随系统 | 用于应用主题设置和用户主题设置为空值时兜底 |
| 回收站保留期 | 30 天 | 用于应用回收站设置和个人回收站设置为空值时兜底 |
| 审计日志保留期 | 180 天 | 用于应用审计设置和个人审计设置为空值时兜底 |
...
面向所有访客展示应用级书签数据，作为产品的公开内容入口。该页面无需登录即可访问，强调浏览、发现、搜索和打开链接。
...
承载超级管理员对应用级书签数据的集中维护、批量处理、标签关联调整和数据整理。该页面仅超级管理员可访问，强调“管”和“改”。
...
- 支持批量设置展示状态（公开/隐藏）。
- 未登录访客访问该页面时，应跳转登录页；普通用户访问该页面时，应返回 403 页面或无权限提示页。

## 7.5 应用标签管理页

### 页面目标

承载超级管理员对应用级标签的创建、编辑、删除和排序管理。该页面仅超级管理员可访问。
...
- 标签排序后，应用书签页与应用书签管理页的标签展示顺序同步更新。

## 7.6 应用导入导出页

### 页面目标

承载超级管理员对应用级书签数据的导入、导出与结果核对。
...
- 导入重复 URL 时直接跳过：重复项按失败记录，其余合法数据继续导入。
...
- 关键策略变更需要二次确认。
- 配置生效后应提示影响范围与生效时间。
- 系统设置字段未设置时，页面读取并展示系统默认设置值；仅在超级管理员主动保存后写入应用设置数据域。

### 配置项明细

| 配置分组 | 配置项 | 可选值/格式 | 默认值 | 生效范围 | 备注 |
|---|---|---|---|---|---|
| 应用主题设置 | 界面主题 | 浅色 / 深色 / 跟随系统 | 空值（读取时回退系统默认设置） | 应用书签页 | 系统设置字段 |
| 应用数据策略 | 回收站保留期 | 7 天 / 30 天 / 90 天 / 永久保留 | 空值（读取时回退系统默认设置） | 应用级回收站 | 系统设置字段 |
| 应用数据策略 | 审计日志保留期 | 30 天 / 90 天 / 180 天 / 365 天 | 空值（读取时回退系统默认设置） | 应用审计日志 | 系统设置字段 |
...
- 配置保存后提示生效状态。
- 系统设置字段未设置时，页面读取并展示系统默认设置值；仅在当前用户主动保存后写入用户设置数据域。

### 配置项明细

| 配置分组 | 配置项 | 可选值/格式 | 默认值 | 生效范围 | 备注 |
|---|---|---|---|---|---|
| 账户信息 | 登录邮箱 | 只读 | 当前登录邮箱 | 当前用户 | 仅展示，不支持在站内修改 |
| 账户安全 | 修改密码 | 原密码 + 新密码 + 确认密码 | 无 | 当前用户 | 修改成功后当前会话保持登录，其他设备会话按安全策略失效 |
| 账户安全 | 注销账户 | 二次确认操作 | 关闭 | 当前用户 | 注销后个人数据按数据策略处理 |
| 主题设置 | 界面主题 | 浅色 / 深色 / 跟随系统 | 空值（读取时回退系统默认设置） | 当前用户全站界面 | 系统设置字段，对应 6.9.3 |
| 个人数据策略 | 回收站保留期 | 7 天 / 30 天 / 90 天 / 永久保留 | 空值（读取时回退系统默认设置） | 当前用户个人回收站 | 系统设置字段 |
| 个人数据策略 | 审计日志保留期 | 30 天 / 90 天 / 180 天 / 365 天 | 空值（读取时回退系统默认设置） | 当前用户个人审计日志 | 系统设置字段 |
...
- 未登录访客访问应用管理页（应用书签管理、应用标签管理、应用导入导出、应用回收站、应用设置）时，需要跳转到登录页。
- 登录页和注册页允许未登录访问。

### 9.6 普通用户访问

- 普通用户登录后可访问应用书签页，并可查看、搜索、筛选、打开、复制公开书签。
...
- 删除标签不删除书签，仅解除关联关系。
- 删除用户级书签进入用户个人回收站；删除应用级书签进入应用级回收站。
...
- URL 是判断重复书签的主要依据。
- 同一数据范围内，书签 URL 必须唯一，不允许保留重复副本（用户级范围按当前用户隔离，应用级范围为全应用）。
- 应用级书签与用户级个人书签属于不同数据范围，二者 URL 重复不视为冲突。
- 应用级书签保存到用户个人书签库后，形成独立的用户级个人书签副本，采用复制而非引用，后续双方编辑、隐藏或删除互不影响。
...
- 导入重复 URL 时直接跳过：重复 URL 计入失败，不覆盖、不创建副本，且不影响其他合法数据写入。
- 导入完成后需要提供导入结果摘要。
...
- 密码类信息不得明文展示。
- 密码不得明文传输；所有密码提交、登录与修改密码，请求中携带的密码必须加密。
- 密码不得明文存储；服务端必须采用带盐哈希等安全方式存储密码，不允许可逆明文还原。
...
- 未登录访客可以访问应用书签页。
- 应用书签页包含顶部区域、公开标签侧边栏和面向展示的内容区域。
- 应用书签页以浏览、搜索、筛选、打开链接为主要体验。
