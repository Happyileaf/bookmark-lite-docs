...
- 书签和标签通过数据范围区分应用级公开数据与用户级个人数据，两类数据字段结构和关联规则保持一致。
...
产品权限采用 RBAC 模型，系统内仅定义两类角色：普通用户、超级管理员。

| 类型 | 身份/角色 | 可访问页面 | 可执行操作 |
|---|---|---|---|
| 未登录访问状态 | 访客（非 RBAC 角色） | 应用公开书签页、登录页、注册页 | 浏览、搜索、筛选、打开和复制应用级公开书签 |
| RBAC 角色 | 普通用户 | 应用公开书签页、用户书签页、用户书签管理页、设置页、个人回收站页 | 浏览应用级公开书签；浏览和管理自己的用户级书签、用户级标签、个人回收站和基础设置；将应用级公开书签保存为个人书签副本 |
| RBAC 角色 | 超级管理员 | 应用公开书签页、用户书签页、用户书签管理页、应用书签管理页、设置页、个人回收站页、应用回收站页 | 拥有普通用户的全部用户级功能；额外可以创建、编辑、删除和维护应用级公开书签与应用级公开标签，并管理应用回收站 |

角色分配规则：

- 用户注册成功后，系统自动分配 RBAC 角色为普通用户。
- 超级管理员角色不通过前端注册或页面操作授予，只能通过数据库操作完成角色提升或回收。
- 数据库角色变更后，下次登录或令牌刷新后生效（具体生效机制以技术实现为准，但必须保证最终一致）。

### 9.2 用户数据隔离

- 普通用户和超级管理员都只能查看和管理自己的用户级个人书签与个人标签，用户之间个人数据不可互访。
- 应用级公开书签和应用级公开标签对所有访客可见。
- 应用级公开书签和应用级公开标签仅超级管理员可管理。
- 超级管理员在管理应用级数据时，不应越权访问或修改其他用户的个人数据。
...
- 未登录访客访问应用书签管理页时，需要跳转到登录页或无权限提示页。
- 登录页和注册页允许未登录访问。
